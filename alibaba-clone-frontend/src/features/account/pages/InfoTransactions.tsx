...
import { getMyTransactions, getProfile, topUp } from "@/api/features/accountApi";
    ...
    <div className="flex flex-col gap-4">
      <div className="bg-white rounded-lg shadow px-6 py-4 min-h-[64px] flex items-center justify-between">
        <div className="text-lg font-semibold text-gray-800 flex items-center">
          Current Balance: <span className="text-blue-700 ml-1">{balance.toLocaleString()} $</span>
        </div>
        <Button variant="outline" className="w-fit flex items-center h-10" onClick={() => setTopUpOpen(true)}>
          Increase Balance
        </Button>
        <TopUpModal open={topUpOpen} onClose={() => setTopUpOpen(false)} topUp={topUpData} onSave={handleTopUp} />
      </div>
      {loading ? (
